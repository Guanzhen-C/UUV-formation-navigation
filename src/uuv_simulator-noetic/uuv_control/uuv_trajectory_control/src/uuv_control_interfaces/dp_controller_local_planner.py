    ...
    def get_idle_circle_path(self, n_points, radius=30):
        ...
        * `radius` (*type:* `float`): Circle radius in meters
        ...
        if self._idle_circle_center is None:
            ...
            # on the vehicle's position and heading. The Z coordinate is 
            # preserved from the vehicle's current depth.
        ...
        * `t` (*type:* `float`): Timestamp
        ...
        if not self._station_keeping_on and self._traj_running:
            ...
            if self._traj_running and self._traj_interpolator.has_finished():
                # Trajectory lap ended, restart it immediately for a seamless loop.
                self._logger.info(rospy.get_namespace() + ' - Trajectory lap completed! Restarting...')

                # Force recalculation of the circle with the current Z value.
                self._idle_circle_center = None
                
                wp_set = self.get_idle_circle_path(150, self._idle_radius)
                wp_set = self._apply_workspace_constraints(wp_set)
                
                if wp_set.is_empty:
                    self._logger.error('Could not generate new idle path, entering station keeping.')
                    self._start_count_idle = rospy.get_time()
                    self.set_station_keeping(True)
                    self.set_automatic_mode(False)
                    self.set_trajectory_running(False)
                else:
                    # Set the new trajectory and reset the start time.
                    # The vehicle continues in automatic mode without stopping.
                    self._traj_interpolator.set_waypoints(wp_set, self.get_vehicle_rot())
